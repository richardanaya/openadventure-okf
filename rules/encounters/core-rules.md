...
1. **Set the Stage**: The Adventure Guide describes where the Encounter takes place, tells the players which NPCs and Adversaries are present, identifies the Challenges (at least those the characters are aware of), and explains the objectives for the Encounter. They also set the Round length.
2. **Declare Starting Positions**: The players describe where their characters are when the Encounter begins, what they are doing, and what gear they are carrying. The Adventure Guide lets the players know the location of important NPCs and Adversaries.
